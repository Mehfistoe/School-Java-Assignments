  ...
  private static void createShapes() {
    for (int i = 0; i < shapes.length; i++) {
      ...
      if (Math.random() <= 0.33)  {
    	...
    	// Generate a triangle with a random size and position
    	int side = generateRandomInt(MIN_SIZE, MAX_SIZE);
    	int x = generateRandomInt(0, WINDOW_SIZE - side);
    	int y = generateRandomInt(0, WINDOW_SIZE - side);
    	shapes[i] = new Triangle(x,y,color,side);
      }
    }
    }
